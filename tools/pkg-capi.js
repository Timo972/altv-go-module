...
const definitionRegex = /*/^[\/]{0,0}EXPORT\s([a-zA-Z0-9\s\*]{1,}[\s\*]{1,1})([A-Z]{1,1}[a-zA-Z]+)_([a-zA-Z0-9]+)\(([a-zA-Z0-9\s\*\,]{0,})\);/gm*/ /EXPORT\s([a-zA-Z0-9\s\*]{1,}[\s\*]{1,1})([A-Z]{1,1}[a-zA-Z]+)_([a-zA-Z0-9]+)\(([a-zA-Z0-9\s\*\,]{0,})\);/gm
// match: whole line
// group 1: return type
// group 2: class name
...
const typedefRegex = /typedef\s[a-z]+\s[a-zA-Z0-9\s]+{[^}]+}[\s]{0,1}[a-zA-Z0-9]+;/gm;
...
async function main() {
  ...
  files.forEach((p) => {
    if (!fs.existsSync(p)) {
      throw new Error(`Could not find file: '${p}' tool must be run in project root`);
    }
  })

  // filter c-api definitions
  const res = (await readDir(files[1])).filter((f) => f.endsWith(".h")).map(f => path.join(files[1], f));
  if (res.length < 1)
  ...
  const partials = await Promise.all(res.map(async (fp) => {
